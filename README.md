...
### Required accepted length, and what batch size does to it

![breakeven](figs/breakeven.png)

*(generated by `breakeven_plot.py`; derivation independently verified by a
second model — including one bug it and we caught convergently)*

**How long must the stale accepted length be?** Two lockstep-batch models
(b sequences decode in step; the batch skips JIT drafting only when every
sequence's outcome is pre-speculated, probability q = p_hit^b):

| model | miss round | break-even |
|---|---|---|
| all-or-nothing (SSD-favorable) | everyone redrafts fresh — identical to a sync round, so misses cancel | `τ_s* = τ_f · t_v/(t_v+t_d)` — **independent of p_hit and b** (any q > 0) |
| mixed / lockstep (= structure of SSD paper Corollary 16) | hitting sequences keep stale proposals, missing ones JIT; all wait | `τ_s*(b) = τ_f · [1 − p^(b−1) · t_d/(t_v+t_d)]` — **rises with b toward τ_f** |

Plugging in the A10 measurements at b = 1 (both models agree there):

| | τ_fresh | required τ_stale | as ratio | measured τ_stale |
|---|---|---|---|---|
| DFlash t=1.0 | 4.35 | **3.63** | 0.836 | 2.55–2.59 |
| DSpark t=1.0 | 4.98 | **3.99** | 0.801 | 2.94–3.07 |
| DFlash t=0.0 | 4.64 | **3.88** | 0.836 | 2.71–2.79 |
| DSpark t=0.0 | 5.11 | **4.16** | 0.814 | 3.01–3.17 |

The largest measured stale τ (3.17) is below the smallest requirement (3.63)
— every pairing fails, and closing the gap needs a **+31–42%** uplift in
stale accepted length (the retraining target).

**What batch size does.** In the all-or-nothing model batch size never flips
the sign of the verdict — it only shrinks the magnitude, roughly like
q = p^b (at p_hit = 0.9: half the effect by b ≈ 7, ~3% left at b = 32). In
the mixed model — the one matching the paper's Corollary 16 — the bar
*additionally rises* with b: at p_hit = 0.9, x = 0.25 the required ratio is
0.80 / 0.90 / 0.96 at b = 1 / 8 / 16 and → 1.0 as b → ∞. Either way, SSD
for these drafts is a **b ≲ 8 proposition even after a successful retrain**;
right panel shows both effects (the r = 0.95 curve crosses below parity near
b ≈ 14).

**A hardware-normalized impossibility.** With S ≤ F the latency gain is
bounded by `K ≤ 1 + t_d/t_v ≤ 1.25` at our time ratios. Per-GPU parity for a
1 target + 1 draft deployment requires K > 2 — mathematically unreachable
for these block drafts regardless of acceptance; even the paper's 4+1
configuration needs K > 1.25, right at our ideal ceiling.

Model limits (flagged in review): t_d/t_v is held constant in b (a roofline
argument says the verifier saturates first on A10, which would raise the bar
further — measure A(b), D(b) per batch to locate the real knee); lockstep
full-batch stall is assumed (an engine with faster backup paths or per-
sequence scheduling can land between the two thresholds); the all-or-nothing
threshold is the lower of these two models, not a universal lower bound.
